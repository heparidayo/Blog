---
title: '[UE5] 0-1. 개발 환경 구축부터 네트워크 세션 기초 설계까지'
published: 2026-04-03
description: '(이전 게시글 0번 1번 통합) UE5.7 기준 데디케이티드 서버, GAS, Enhanced Input 환경 구축과 세션 기반 네트워크 연결 테스트 통합 정리'
image: ''
...
언리얼 프로젝트를 본격적으로 개발하기 전, 데디케이티드 서버 구조를 상정하고 GAS를 적용한 초기 환경 구축부터 다중 접속을 위한 네트워크 세션 설계까지의 과정을 정리했다.

## 1. 데디케이티드 서버 빌드 타겟과 로그 설정

언리얼 엔진의 기본 템플릿은 에디터와 클라이언트 빌드만 제공한다. 서버 전용 로직을 검증하기 위해 별도의 `.Target.cs` 설정이 필요하다.

일반적으로 `Shipping` 빌드에서는 성능 최적화를 위해 모든 로그가 제거된다. 하지만 데디케이티드 서버 운영 중 발생하는 예외 상황을 추적하기 위해서는 로그가 필요하므로, 서버 타겟 설정에서 명시적으로 로그를 활성화해야 한다.
...
협업 및 버전 관리를 위해 `.gitignore`와 `.gitattributes`를 설정한다.
...
콘솔 창(`~`)에서 `ShowDebug AbilitySystem` 명령어를 입력했을 때, 현재 캐릭터가 가진 태그, 이펙트, Attribute(체력, 마나 등)을 오버레이로 확인하기 위해 `DefaultGame.ini`에 다음 설정을 추가한다.
...
기본 템플릿의 `BP_ThirdPersonCharacter` 블루프린트를 재사용하거나 에셋 경로를 하드코딩하는 대신, BP가 상속받는 기반이 되는 C++ 기반 클래스(`ACoopGameCharacter`)를 직접 구현하여 로직과 시각적 에셋을 분리한다.

카메라 세팅, 이동 컴포넌트 처리, Enhanced Input의 IMC(Input Mapping Context) 바인딩과 같은 로직은 C++에서 담당하고,    

이를 상속받은 BP에서는 메쉬나 애니메이션 블루프린트(AnimBP) 등 시각적 에셋만 할당하여 로직과 에셋을 분리한다.

> **새로운 입력(단축키, 액션 등) 추가 프로세스**

1. **IA 에셋 생성:** 에디터에서 `IA_Jump` 등의 Input Action 에셋 생성.
2. **IMC 매핑:** `IMC_Default`에 IA를 추가하고 입력 키 매핑 (필요시 Modifiers/Triggers 세팅).
3. **.h 변수 노출:** 헤더 파일에서 `UPROPERTY`를 사용해 BP에 에셋을 연결할 슬롯 생성.
4. **.cpp 바인딩 및 구현:** 소스 파일에 동작 함수를 구현하고 `BindAction`으로 IA와 매핑.
5. **BP 할당:** C++을 상속받은 BP를 열고 노출된 슬롯에 IMC와 IA 에셋 할당.

이때 클래스 헤더에 에셋 참조를 선언할 때 사용하는 `UPROPERTY` 매크로는 에디터에 변수를 노출하고 직렬화하기 위해 필수적이다. (유니티의 `[SerializeField]` 특성(Attribute)와 유사하다. 인스펙터(디테일 패널)에 변수를 노출하고 값을 직렬화하여 저장할 수 있게 해준다.)   
...
기본적인 개발 환경과 로컬 캐릭터 제어 구조가 잡혔다면, 다음은 다중 접속을 위한 네트워크 설계다. 이를 위해 언리얼의 리플리케이션(Replication) 개념을 짚고 넘어가자

### 서버-클라이언트 구조

* **[Dedicated Server]:** 게임 로직의 모든 권위(Authority)를 가진다. 화면을 렌더링하지 않으며, 게임 상태만 관리하고 이를 클라이언트에 복제(Replicate)한다.
...
추후 GAS와 연동될 플레이어의 생존 여부 등을 관리한다.
...
## 8. 온라인 서브시스템 (OSS) 및 세션 구현

현재는 로컬 LAN 테스트를 위해 `NULL` 서브시스템을 사용하며, 추후 Steam OSS로 교체할 수 있는 구조로 설계한다. *(에디터 Plugins에서 `Online Subsystem` 및 `Online Subsystem Utils` 활성화 필수)*
...
세션 로직은 씬 전환(Level Travel) 시에도 파괴되지 않고 유지되는 `UGameInstance`에 구현하는 것이 안정적이다.
...
작성된 코드가 데디서버 환경에서 정상적으로 동작하는지 확인한다.

### 방법 1: 에디터 내 테스트 (PIE)

* **설정:** `Play As Listen Server`, `Number of Players: 2+`
* **결과:** 클라이언트 창이 2개 이상 생성되며 캐릭터가 동일 맵에서 동기화되는지 확인.
